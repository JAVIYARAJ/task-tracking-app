...
class DatabaseHelper{
  ...
  static Future<sql.Database> db() async{
    return sql.openDatabase('flutterDb',version: 1,onCreate: (sql.Database database,int version) async{
  ...
  static Future<void> createItem(String title,String description) async{
    ...
    final id=await db.insert('items',data,conflictAlgorithm: sql.ConflictAlgorithm.replace).then((value) => debugPrint("insert successfully"));
  }
  ...
  static Future<String> _translateText(String message, String languageCode) async {
    final text = await translator.translate(
        message, from: "auto", to: languageCode);
    return text.text;
  }

  static Future<List<Map<String,dynamic>>> getItemsWithSpecificLanguage(String filter,String languageCode) async{
    final db=await DatabaseHelper.db();
    ...
    for (int i = 0; i < text.length; i++) {
      ...
      final description = await _translateText(
          text[i]["description"], languageCode);
  ...
  static Future<void> updateItem(int id,String title,String description) async{
    ...
    final result=await db.update('items', data,where: "id = ?",whereArgs: [id]).then((value) => debugPrint("update successfully"));
  }

  static Future<void> deleteItem(int id) async{
    final db=await DatabaseHelper.db();
    try{
      await db.delete('items',where: "id = ?",whereArgs: [id]).then((value) => debugPrint("delete successfully"));
    }catch(e){
